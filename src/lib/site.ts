...
export const landingPages: Record<string, LandingPageData> = {
  home: {
    path: "/",
    label: "中文用户优先 · 百度和 Google 都看得懂的总览页",
    title: "中国可用的稳定 VPN 推荐",
    description:
    ...
    keywordText: "中国可用 vpn / 稳定 vpn 推荐 / 中国网络环境 vpn",
    searchIntent: "适合搜“稳定 VPN 推荐”“中国可用 VPN”的用户，先看总览，再按场景进入对应子页。",
    openingAnswer: [
    ...
    recommendationReason:
      "这页更像入口页：先给出适合中文用户的判断框架，再通过子页把中国使用、免费试用和无日志三个主题拆开讲清楚，兼顾百度的直给匹配和 Google 更看重的完整语义。",
  ...
  china: {
    ...
    searchIntent: "适合搜“中国好用 VPN”“适合中国使用的 VPN”的用户，搜索意图偏向连接是否稳定、值不值得试。",
    openingAnswer: [
    ...
    recommendationReason:
      "这页更适合承接百度里的直给搜索词，也能覆盖 Google 上关于“best VPN for China”的中文语义需求。页面重点不是堆“中国”两个字，而是把中国使用场景真正拆开讲。",
  ...
  trial: {
    ...
    searchIntent: "适合搜“免费试用 VPN”“先试再买 VPN”的用户，核心诉求是降低试错成本。",
    openingAnswer: [
    ...
    recommendationReason:
      "这个主题既适合 Google 对真实使用建议的理解，也很适合百度里“免费试用 VPN”这种直接长尾词。把“试用期应该看什么”写清楚，页面会更像内容页，而不是纯导流页。",
  ...
  nolog: {
    ...
    keywordText: "无日志 vpn / 不留日志 vpn / 隐私保护 vpn",
    searchIntent: "适合搜“无日志 VPN”“不留日志 VPN”的用户，搜索重点偏向隐私与长期使用安心感。",
    openingAnswer: [
    ...
    recommendationReason:
      "这一页既能覆盖百度里的“无日志 VPN”长尾，也能满足 Google 更偏向“为什么无日志值得关注”的信息需求，让页面不只是换个关键词，而是真的围绕隐私主题展开。",
...
export const topicHubSections = [
  {
    title: "先按搜索意图选入口，而不是一上来就看一堆词",
    body: "如果你搜的是“中国好用 VPN”，说明你更看重连接稳定性；如果你搜的是“免费试用 VPN”，说明你更想降低试错成本；如果你搜的是“无日志 VPN”，说明你对隐私更敏感。把这些意图分开看，会更高效。",
  },
  {
    title: "百度更看重中文直给结构，Google 更看重内容完整度",
    body: "所以这个专题页的作用，就是把几个核心主题清楚地串起来：标题更直接、正文更完整、内链更清晰，让两个搜索引擎都更容易判断页面主题。",
  },
  {
